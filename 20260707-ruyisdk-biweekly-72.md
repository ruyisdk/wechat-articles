...
各位 RuyiSDK 的开发者伙伴，大家好！过去两周，RuyiSDK 各核心组件持续稳步推进，本期有几项亮点值得关注。

运行时方面，V8 持续完善 ZVFH 半精度浮点扩展支持，为 Liftoff WebAssembly SIMD 开启 ZVFH 并实现 fp16 舍入运算指令；OpenJDK 的 JEP 401（值类与对象）移植再进一步，已完成 RISC-V 平台模板解释器模块支持，在纯解释模式下跑通相关测试用例，开始推进 C1 与 C2 JIT 模块适配。

工具链方面，GCC 完成 ruyisdk-gcc 16.1 与 binutils 2.46 的整合，支持所有 2026 年 6 月前已批准的 RISC-V 扩展及全面 RV Profiles；GLIBC 在玄铁 C908 平台完成 memset、memmove 初步调优；LLVM 提供 RISC-V msan 功能并合入多项 P 扩展 codegen 与 intrinsic 支持。

开发者工具方面，软件源更新 GNU 与 LLVM 工具链至 0.20260625.0 版本，新增 SpacemiT K3 Pico-ITX 设备支持；VSCode 与 Eclipse 插件新发布 0.1.5 版本。

更多进展详见正文，欢迎前往[官网下载页面](https://ruyisdk.org/downloads)下载RuyiSDK试用并在 [GitHub](https://github.com/ruyisdk/) 或[社区](https://ruyisdk.cn/)中反馈建议，期待更多伙伴一起共建 RISC‑V 软件开发生态。

⏬ 包管理器和IDE插件工具都提供了多种下载或安装方式，您可以任意选择一种：

* RuyiSDK 包管理器 0.50.0

  * 从 [PyPI](https://pypi.org/project/ruyi/0.50.0/) 安装：`pip install ruyi`
  * 手动下载安装：
    * [GitHub Releases](https://github.com/ruyisdk/ruyi/releases/tag/0.50.0)
    * [ISCAS 镜像源](https://mirror.iscas.ac.cn/ruyisdk/ruyi/tags/0.50.0/)
